...
type Helpers = {
  block: (name: string) => string | TemplateStringReturnValue;
  children: () => string;
};
...
class Macro<T = unknown> {
  ...
  public helpers: Helpers = {
    block: (name: string): string | TemplateStringReturnValue => {
      this.hasBlocks = true;
      return this.blocks.get(name) ?? `@__block:${name}`;
    },
    children: () => {
...
export const raw = (value: string): RawString => ({ value, __type: "raw_string"})
